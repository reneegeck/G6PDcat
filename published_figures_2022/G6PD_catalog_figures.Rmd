...
```{r load_files}
g6pd_cat <- read.table("fig_G6PD_variant_catalog.txt", sep="\t", header=T)
g6pd_cat_noB <- filter(g6pd_cat, !is.na(hg19_core))

g6pd_cat_mol <- read.table("fig_mol_act.txt", sep="\t", header=T)
g6pd_cat_mol_info <- read.table("fig_G6PD_mol_info.txt", sep="\t", header=T)

acmg_counts <- read.table("fig_acmg_counts.txt", sep="\t", header=T)
...
#make the df
variants <- c(77, 159, 62, 223, 54, 20, 39, 345, 84, 3, 10, 147)
...
fig_3a <- ggplot(data=g6pd_cat_single, aes(x=position, y=activity_avg, color=phenotype)) +
  ...
  ylab("variant activity") +
  labs(color="most severe phenotype") +
  scale_y_continuous(trans='log10') +
  scale_color_manual(values=who_col)
...
fig_3d <- ggplot(data=subset(g6pd_cat, !is.na(stability)), #only graph if has stability data
  ...
  ylab("variant activity") +
  labs(color="most severe observed\nclinical phenotype") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
fig_3e <- ggplot(data=subset(g6pd_mol_stabnum, !is.na(phenotype)), #only graph if has ptype
  ...
  geom_violin(outlier.alpha=F) + #hide outlier points since already graphed
